...
class Model(nn.Module):
    ...
    def __init__(self, x_dim, h_dim, z_dim, n_batch, device, n_pseudo_inputs, layers, weighted_vp_bool, He_bool):
        ...
        if n_pseudo_inputs > 0:
            ...
            if self.weighted_vp_bool:
                ...
                self.w_pseudo_layer = nn.Linear(self.n_pseudo_inputs, 1)
    ...
    def generative(self, n_digits, pseudo_input_gen=1):
        ...
        if self.vampprior_bool:
            ...
            if n_digits > 1:
                ...
                self.z_L = self.z_L[pseudo_input_gen-1:pseudo_input_gen]
    ...
    def measure_dead_units(self, X, batch_size=None):
        ...
        self.z_dead_units_per_layer = np.zeros((self.layers, X.shape[0], self.z_dim))
        for en, n in enumerate(range(int(X.size(0) / self.n_batch))):
            x = X[n * self.n_batch: (n + 1) * self.n_batch]
            self.forward(x)

            ### Remember the means are saved in reversed format in self.en_q_z_mean_layer
            for en_layer, layer in enumerate(reversed(range(self.layers))):
                self.z_dead_units_per_layer[en_layer, en*self.n_batch: (en+1)*self.n_batch] = self.en_q_z_mean_layer[layer].cpu().data.numpy()

        self.z_variances = np.zeros((self.layers, self.z_dim))
        for layer in range(self.layers):
            self.z_variances[layer] = np.var(self.z_dead_units_per_layer[layer], axis=0)

        if batch_size is not None:
            self.n_batch = temp_n_batch

        return self.z_variances
...
def training_model(model_name, train_data, valid_data, device, layers, x_dim, h_dim, z_dim, n_batch, eta, epochs,
    ...
    for epoch in tqdm_epochs:
        ...
        z_dead_units_per_layer = np.zeros((layers, valid_data.shape[0], z_dim))
        temp_valid_loss, temp_kl_loss_val, temp_re_loss_val = 0, 0, 0
        for en, n in enumerate(range(0, int(valid_data.shape[0] / n_batch))):
            ...
            for en_layer, layer in enumerate(reversed(range(layers))):
                z_dead_units_per_layer[en_layer, en*n_batch: (en+1)*n_batch] = vae.en_q_z_mean_layer[layer].cpu().data.numpy()

        active_units_per_layer = []
        for layer in range(layers):
            active_units_per_layer.append(sum(np.var(z_dead_units_per_layer[layer], axis=0) > 0.01))
        ...
        tqdm_epochs.set_description(
            "Epoch:{} Training Loss:{} Validation Loss:{}".format(epoch, train_loss_lst[-1], valid_loss_lst[-1]))
...
def measure_active_units_model(X, model):
    ...
    for layer in range(model.layers):
        active_units_per_layer.append(sum(model.z_variances[layer] > 0.01))
...
if __name__ == "__main__":
    ...
    pseudo_input_gen = 1
    n_digits = 25
    sqrt_digit = int(np.sqrt(n_digits))
    n_batch = n_digits
    ...
    model_name = ("VAE_" if layers == 1 else "HVAE_L_{}_".format(layers)) + (
        ...
        "Wu_" if warm_up > 0 else "") + ("He" if He_bool else "Xa") #+ "_new"

    print(model_name)

    model = Model(x_dim, h_dim, z_dim, n_batch, device, n_pseudo_inputs, layers, weighted_vp_bool, He_bool)
    model.load_state_dict(torch.load(os.path.join("Results", model_name, "best_model.pickle"), map_location=device))
    model.to(device)

    digits_ind = np.linspace(0, test_data.shape[0]-1, n_digits).astype(int)
    X_test = Variable(torch.tensor(test_data).float().to(device))[digits_ind]
    model.forward(X_test)
    samples = [deepcopy(sample.to("cpu").data.numpy()) for sample in model.p_x_mean]

    fig = plt.figure("Reconstruction Digits")
    ax = fig.add_axes([0., 0., 1., 1., ])
    figure = np.zeros((28*sqrt_digit, 28*sqrt_digit))
    plt.axis('off')
    for i in range(sqrt_digit):
        for j in range(sqrt_digit):
            figure[i * 28: (i + 1) * 28, j * 28: (j + 1) * 28] = samples[i*sqrt_digit + j].reshape(28, 28)
    plt.imshow(figure, cmap='gray')
    plt.savefig(os.path.join("Results", model_name, model_name[:-3] + "_recon.png"))
    plt.close()

    plt.figure("Generated Digits from generative")
    figure = np.zeros((28*sqrt_digit, 28*sqrt_digit))
    plt.axis('off')

    for i in range(sqrt_digit):
        for j in range(sqrt_digit):
            model.generative(1, pseudo_input_gen)
            if i == 0 and j == 0 and n_pseudo_inputs > 0:
                if weighted_vp_bool:
                    w_pseudo_input = torch.argsort(model.w_pseudo_layer.weight, descending=True).squeeze()[pseudo_input_gen-1]
                    sample = model.u[w_pseudo_input].cpu().data.numpy().reshape(28, 28)
                elif n_pseudo_inputs > 0:
                    sample = model.u[pseudo_input_gen-1:pseudo_input_gen].cpu().data.numpy().reshape(28, 28)
                else:
                    model.p_x_mean.cpu().data.numpy().reshape(28, 28)
            else:
                sample = model.p_x_mean.cpu().data.numpy().reshape(28, 28)
            figure[i*28: (i+1)*28, j*28: (j+1)*28] = sample
    plt.imshow(figure, cmap='gray')
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    figManager = plt.get_current_fig_manager()
    figManager.window.showMaximized()
    plt.savefig(os.path.join("Results", model_name, model_name[:-3] + "_gen.png"))
    plt.close()
    ...
    ###
    # for model_dir in os.listdir(os.path.join("Results")):
    #     print(model_dir)
    #     n_pseudo_inputs = 0 if "SG" in model_dir else 500
    #     layers = int(model_dir.split("_")[2]) if "HVAE" in model_dir else 1
    #     He_bool = "He" in model_dir
    #     weighted_vp_bool = "WG" in model_dir
    #     warm_up = 100 if "Wu" in model_dir else 0
    #
    #     model_name = ("VAE_" if layers == 1 else "HVAE_L_{}_".format(layers)) + (
    #         "SG_" if n_pseudo_inputs == 0 else "VP_") + ("WG_" if bool(weighted_vp_bool) else "") + (
    #         "Wu_" if warm_up > 0 else "") + ("He" if He_bool else "Xa")
    #
    #     print(model_name)
    #
    #     model = Model(x_dim, h_dim, z_dim, n_batch, device, n_pseudo_inputs, layers, weighted_vp_bool, He_bool)
    #     model.load_state_dict(torch.load(os.path.join("Results", model_dir, "best_model.pickle"), map_location=device))
    #     model.to(device)
    #
    #     text = "Validation dataset\n"
    #
    #     X_val = Variable(torch.tensor(valid_data).float().to(device))
    #
    #     z_dead_per_layer = model.measure_dead_units(X_val)
    #     for layer in range(layers):
    #         act_units_per_layer = sum(z_dead_per_layer[layer] > 0.01)
    #         print(act_units_per_layer)
    #         text += "Active units for layer:{} are {}\n".format(layer+1, act_units_per_layer)
    #
    #     text += "\nTraining dataset\n"
    #
    #     X_test = Variable(torch.tensor(test_data).float().to(device))
    #
    #     z_dead_per_layer = model.measure_dead_units(X_test)
    #     for layer in range(layers):
    #         act_units_per_layer = sum(z_dead_per_layer[layer] > 0.01)
    #         print(act_units_per_layer)
    #         text += "Active units for layer:{} are {}\n".format(layer+1, act_units_per_layer)
    #
    #     with open(os.path.join("Results", model_dir, "active_units.txt"), "w") as fl:
    #         fl.write(text)
    ###
    ...
    # z_dead_per_layer = model.measure_dead_units(X_test)
    # for layer in range(layers):
    #     print(sum(z_dead_per_layer[layer] > 0.01))
    ###
    ...
    # plt.imshow(figure, cmap='gray')
    # plt.tight_layout()
    # plt.savefig(os.path.join("Results", model_name, "Generative_1_pseudo_input.png"))
    # plt.close()
    ###
